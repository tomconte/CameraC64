#import "GPUImageTransformFilter2.h"

NSString *const kGPUImageTransformVertexShaderString2 = SHADER_STRING
(
 attribute vec4 position;
 attribute vec4 inputTextureCoordinate;
 
 uniform mat4 transformMatrix2;
 uniform mat4 orthographicMatrix2;
 
 varying vec2 textureCoordinate;
 
 void main()
 {
     gl_Position = transformMatrix2 * vec4(position.xyz, 1.0) * orthographicMatrix2;
     textureCoordinate = inputTextureCoordinate.xy;
 }
);

@implementation GPUImageTransformFilter2
...
- (id)init;
{
    if (!(self = [super initWithVertexShaderFromString:kGPUImageTransformVertexShaderString2 fragmentShaderFromString:kGPUImagePassthroughFragmentShaderString]))
    {
        return nil;
    }
    
    transformMatrixUniform = [filterProgram uniformIndex:@"transformMatrix2"];
    orthographicMatrixUniform = [filterProgram uniformIndex:@"orthographicMatrix2"];
